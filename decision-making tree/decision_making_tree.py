...
# the retropy is the last labels, you must classify it unqiue, then calculating the sum total of each other
# the formula of retropy is probability of unique label, then multiply the math.log(probability,2)
# after you calculate each retropy of labels, add it and the sum is this final retropy of this col
def calcShannonEnt(dataSet): # calculate the retropy which is the key for Information-Gain
...
# split the dataSet by index axis, and just retain the one whose value is value that you input
# which meaning return a dataSet just with value in axis column
